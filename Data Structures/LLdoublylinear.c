...
// Creating a structure to store data with a pointer to the next node, and the previous node.
...
/* Function to add a node to an existing or to create a doubly linear linked list if not created.*/
...
/* Function to display all elements present in doubly linear linked list. */
...
/* Function to add a node at the beginning of the doubly linear linked list. */
...
void addBeg()
{
	...
	{
		start->prev = newnode; //Current nodes previous should point to newnode at beginning.
		newnode->next = start;
		newnode->prev = NULL;
		start = newnode;
	}
}
// Function to count the number of elements present in the doubly linear linked list.
